...
\titulo{Estática}
\subtitulo{Mecánica, Vectores, Fuerzas, Equilibrio}
...
\end{multicols*}
    ...
    \newpage
    \titulo{Estática}
    \subtitulo{Momentos de fuerza y fricción}
...
\begin{multicols*}{2}
    ...
    \eqn{Momento respecto a un eje}{
        M^{\vec{F}_A}_{Q\parallel QP} &= \hat{r}_{P/Q} \bullet \vec{M}_Q^{\vec{F}_A}
        = \hat{r}_{P/Q} \bullet \vec{r}_{A/Q} \times \vec{F}_A \\
        \vec{M}^{\vec{F}_A}_{Q\parallel QP} &= |  M^{\vec{F}_A}_{Q\parallel QP} | \hat{r}_{Q/P}
    }
    ...
    \eqn{Centroide en segmento de círculo}{
        \bar{y} = \frac{4nr}{3\pi}\left| \cos{\left( \frac{(n+1)\pi}{2n} \right)} \right|
    }

    \eqn{Centroide en segmento de arco}{
        \bar{y} = \frac{2r \sin{\frac{\theta}{2}}}{\theta}
    }
